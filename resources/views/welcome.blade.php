    ...
    <body class="antialiased">
        ...
        <script>
            ...
            function exportUsers() {
                startTimer();
                const classList =
                    document.querySelector("#export-btn").classList;
                classList.add("disabled-link");
                axios
                    .get("/api/users/export?type=2")
                    .then(({ data }) => {
                        const intervalId = setInterval(() => {
                            axios
                                .get(
                                    `/api/users/tracking-export/${data.tracking_id}`
                                )
                                .then(({ data }) => {
                                    ...
                                    if (data.status) {
                                        ...
                                        window.location.href = `/api/users/download/${data.tracking_id}`;
                                        stopTimer();
                                    }
                                });
                        }, 5000);
